...
// create fizz, buzz, and fizzbuzz arrays and iterate through nums array using for...of
const fizz = [];
const buzz = [];
const fizzbuzz = [];
...
console.log(`this is fizz ${fizz}, and buzz: ${buzz}, and fizzbuzz: ${fizzbuzz}`); // For verification
...
1) Assign the last nested array in the `numArrays` below to a variable named `numList`. As you do this, also fulfill these goals:
   ...
   - Assume you don't know how many nested arrays `numArrays` contains. (I'll use method length to determine this.)
   - Do not alter the original `numArrays` array. (I'll create numList without changing numArrays.)
...
// let's find out how many nested arrays are in numArrays using length property
const numberOfNestedArrays = numArrays.length;
console.log(`numArrays contains ${numberOfNestedArrays} nested arrays.`); // For verification

// let's find the last nested array using length property
const numList = numArrays[numberOfNestedArrays - 1]; // -1 because array indices are zero-based 
console.log('The last nested array in numArrays is:', numList); // For verification
